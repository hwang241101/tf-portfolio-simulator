...
> 상세 설계 의도: [docs/architecture.ko.md](./docs/architecture.ko.md)

---

## 3. 인프라 & 배포

로컬 개발과 **프로덕션에 가까운 배포**를 분리해 구성했습니다. 아래는 당시 프로덕션 구조이며, **현재는 비용 절감을 위해 AWS(EB·RDS) 등 클라우드 리소스를 종료**한 상태입니다.
...
> 상세: [docs/backend.ko.md](./docs/backend.ko.md)
...
`User` → `Portfolio` → **`Allocation`**(목표 비율) / **`Transaction`**(매수·매도 기록), ETF 종목은 **`Asset`** 마스터로 관리합니다. 스키마: `backend/prisma/schema.prisma`
...
| `GET`                     | `/portfolios/:id/summary`           | 요약·CAGR·MDD·`valueHistory`                  |
| `POST`                    | `/portfolios/:id/rebalance/preview` | 리밸런싱 계획만 반환                          |
| `POST`                    | `/portfolios/:id/rebalance/apply`   | 계획을 `Transaction`으로 저장                 |
| `GET` / `POST` / `DELETE` | `/transactions`                     | 거래 조회·추가·삭제 (`portfolioId` 쿼리)      |
| `GET` / `POST` / `DELETE` | `/allocations`                      | 목표 배분 조회·갱신·삭제 (`portfolioId` 쿼리) |
| `GET`                     | `/assets`                           | ETF 종목 마스터                               |
...
| 5    | `docs/assets/screenshots/06-risk-analysis-metrics.png`             | 리스크 분석 — 지표 카드·차트               |
| 6    | `docs/assets/screenshots/07-risk-analysis-ai-recommendation.png`   | 리스크 분석 — AI Recommendation 카드       |
...
> 지표 정의·공식·AI 규칙: [docs/metrics-and-recommendations.ko.md](./docs/metrics-and-recommendations.ko.md)

> 프론트 상세: [docs/frontend.ko.md](./docs/frontend.ko.md)
...
> 용어·공식·임계값·코드 경로: [docs/metrics-and-recommendations.ko.md](./docs/metrics-and-recommendations.ko.md)  
> 리밸런싱 수식: [docs/business-logic.ko.md](./docs/business-logic.ko.md)

### UX 설계 — 『생각하지 마』 관점을 일부 반영

금융 도메인 UI도 **처음 보는 사람이 비율·용어 때문에 멈추지 않게** 하는 것이 중요하다고 보고, Steve Krug 『생각하지 마』(_Don't Make Me Think_)의 **“사용자를 고민하게 만들지 말라”** 는 관점을 **그대로 옮기기보다**, 이 프로젝트의 데모 흐름에 맞게 **일부만** 적용했습니다.

#### 투자 성향 프리셋（積極 / 均衡 / 安定）

|             |                                                                                                                                                  |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **개선 전** | ETF 비율을 사용자가 직접 입력 → 투자 경험이 적으면 “무슨 비율이 맞지?”에서 **시뮬레이션 시작 전에 이탈**                                         |
| **개선 후** | 안정·균형·공격 **프리셋** 선택 → 목표 배분(Allocation) 일괄 반영 → **바로 결과·리밸런싱 화면**으로 이동                                          |
| **UX 관점** | 사용자는 “비율 표를 채우러” 온 게 아니라 **성향에 따른 결과를 보러** 온다. 초기 선택 비용을 줄이고 **빠르게 ‘움직이는 숫자’** 에 도달하도록 설계 |

#### 금융 지표 툴팁（CAGR · MDD · Sharpe 등）

|             |                                                                                                                 |
| ----------- | --------------------------------------------------------------------------------------------------------------- |
| **개선 전** | CAGR·MDD·Sharpe 등 **약어 카드만** 표시 → 검색 없이는 의미 파악이 어려움                                        |
| **개선 후** | 대시보드·리스크 분석 카드에 **설명 툴팁** 추가 → 화면 안에서 짧은 정의·맥락 확인                                |
| **UX 관점** | “이게 뭐지?” 순간 **흐름이 끊긴다**. 자주 보이지만 낯선 용어는 **필요한 위치에서 즉시 설명**해 인지 부담을 줄임 |
...
- **“AI” 라벨 vs 규칙 엔진** → UI는 짧게 “AI”, README·설명에서는 **규칙 기반**임을 명시해 오해 방지
...
기술 스택·선정 이유는 [§2 기술 선택](#2-기술-선택-why--기대-효과)을 참고하세요. 이 규모에서는 **단일 API + REST**로 충분했고, 마이크로서비스·GraphQL은 설명 비용만 커진다고 느꼈습니다.

---

### 6.2 화면 구성: 데모 순서에 맞춰 페이지를 나눈다

**한 일**

- **`/dashboard`** — 요약·CAGR/MDD·구성 차트·리스크 한 줄 인사이트로 “지금 상태”를 먼저 보여 줍니다.
- **`/portfolio`** — 프리셋(積極/均衡/安定) 변경 → 목표 배분 갱신, 거래 확인·추가가 **메인 조작 화면**입니다.
- **`/rebalancing`** — **Preview(계획만)** 와 **Apply(기록 반영)** 를 나눠, “계산 결과”와 “시뮬레이션 반영”을 구분했습니다.
- **`/risk-analysis`** — Volatility·Sharpe·Calmar 등 2차 지표와 **AI Recommendation** 카드로 “숫자 뒤의 해석”을 붙였습니다.
- 사이드바 **포트폴리오 ID 전환**으로 전 화면 데이터가 같이 바뀌게 해, 여러 시드 포트폴리오를 데모하기 쉽게 했습니다.

**배운 점**

- 리밸런싱처럼 **위험한 동작(Apply)** 은 버튼·단계를 나누지 않으면 “실제 주문인가?” 오해가 생기기 쉽습니다.

---

### 6.3 UI 정리: 숫자와 흐름이 한눈에 보이게

**한 일**

- MUI **카드·테이블·차트**로 지표·비율·BUY/SELL 목록을 같은 톤으로 맞췄고, 프리셋 변경은 **확인 다이얼로그**로 한 번 더 걸었습니다.
- 리밸런싱 Preview 표에서 **목표 비율 vs 현재 비율·수량·금액**이 같이 보이게 했고, Apply 후에는 토스트·화면 변화로 “반영됐다”는 피드백을 줬습니다.
- 리스크 화면은 지표 카드 → 성장/낙폭 차트 → AI Recommendation 순으로 **위에서 아래로 읽히게** 배치했습니다.

**배운 점**

- 금융·시뮬레이터 UI는 로직만 맞는 것보다, 이용자가 **BUY/SELL·비율 변화를 눈으로 따라갈 수 있는지**가 신뢰에 더 크게 작용합니다.
- 스크린샷·데모 영상을 README에 넣을 때는 **시크릿 창·북마크바 숨김·시드 데이터 로드된 상태**처럼, UI polish가 README 품질까지 이어진다고 느꼈습니다.
- mock 가격·세금 미반영 같은 한계는 화면 톤(“시뮬레이션”“Preview”)과 문서에 같이 쓰는 편이 “실거래 앱”과 혼동되지 않습니다.

---

### 6.4 배포는 “코드 올리기” 한 번으로 끝나지 않는다
...
### 6.5 프로덕션 장애는 로그를 봐야 하고, 로그도 항상 남지는 않는다
...
### 6.6 HTTPS 사이트에서 HTTP API를 직접 부르면 브라우저가 막는다
...
### 6.7 환경 변수는 “이름”과 “노출 범위”를 구분해야 한다
...
| 문서                                                                          | 내용                                                |
| ----------------------------------------------------------------------------- | --------------------------------------------------- |
| [architecture.ko.md](./docs/architecture.ko.md)                               | 설계 의도·데이터 흐름                               |
| [business-logic.ko.md](./docs/business-logic.ko.md)                           | 리밸런싱 수식                                       |
| [metrics-and-recommendations.ko.md](./docs/metrics-and-recommendations.ko.md) | CAGR·MDD·Volatility·Sharpe, AI Recommendation(규칙) |
| [backend.ko.md](./docs/backend.ko.md)                                         | API·배포                                            |
| [frontend.ko.md](./docs/frontend.ko.md)                                       | 화면·상태                                           |
| [assets/README.ko.md](./docs/assets/README.ko.md)                             | 스크린샷·동영상 파일명·촬영 가이드                  |

> 각 `docs/*.ko.md`는 한국어, `docs/*.md`는 일본어입니다. 루트 README 일본어 버전은 [README.md](./README.md)입니다.
