...
|               |                                                                                                                                                                                         |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Live Demo** | [Vercel フロント](https://tf-portfolio-simulator.vercel.app) _(AWS バックエンド終了 — **`NEXT_PUBLIC_USE_MOCK_API=true`** で mock JSON デモ。下記 **デモ動画**・**スクリーンショット**・[ローカル実行](#ローカル実行) を参照)_ |
...
| **環境変数**          | EB: `DATABASE_URL` / Vercel: `API_PROXY_TARGET` または **`NEXT_PUBLIC_USE_MOCK_API=true`**（バックエンドなし画面デモ） |
...
### UX 設計 — 『Don't make me think』の視点を一部反映

金融ドメインの UI でも、**初見の人が比率や用語で止まらないこと**が重要だと考え、Steve Krug『Don't Make Me Think』（邦題『ユーザーを考えさせるな』など）の **「ユーザーを悩ませるな」** という考え方を、**そのまま写すのではなく**本プロジェクトのデモの流れに合わせて**一部だけ**取り入れました。

#### 投資スタイルプリセット（積極 / 均衡 / 安定）

|               |                                                                                                                                                      |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **改善前**    | ETF 比率をユーザーが直接入力 → 経験が浅いと「どの比率が正しい？」で**シミュレーション開始前に離脱**                                                  |
| **改善後**    | 安定・均衡・積極の**プリセット**選択 → 目標配分（Allocation）を一括反映 → **すぐ結果・リバランス画面**へ                                             |
| **UX の観点** | ユーザーは「比率表を埋めに」来たのではなく、**スタイルに応じた結果を見に**来ている。初期の選択コストを下げ、**早く「動く数字」**に到達できるよう設計 |

#### 金融指標ツールチップ（CAGR · MDD · Sharpe など）

|               |                                                                                                              |
| ------------- | ------------------------------------------------------------------------------------------------------------ |
| **改善前**    | CAGR・MDD・Sharpe など**略語カードのみ** → 検索なしでは意味が伝わりにくい                                    |
| **改善後**    | ダッシュボード・リスク分析のカードに**説明ツールチップ** → 画面内で短い定義・文脈を確認                      |
| **UX の観点** | 「これ何？」の瞬間に**流れが切れる**。よく出るが馴染みのない用語は**必要な場所で即説明**し、認知負荷を下げる |
...
# バックエンドなし UI のみ: NEXT_PUBLIC_USE_MOCK_API=true npm run dev
```

**Vercel（mock デモ）** — Project → Settings → Environment Variables:

| 変数 | 値 | 説明 |
| ---- | -- | ---- |
| `NEXT_PUBLIC_USE_MOCK_API` | `true` | ブラウザが `/api-proxy` の代わりに内蔵 mock JSON を使用 |
